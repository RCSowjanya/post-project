...
const SideNavbar = () => {
  ...
  return (
    <div className="">
      <div className={`bg-white  pt-4 ${open ? "w-64" : "w-0"} `}>
        <IoMdArrowRoundForward
          className={`bg-[#C9252C] ml-4 p-1 z-[999] text-white text-3xl rounded-full  -right-3 top-9 cursor-pointer ${
            !open && "rotate-180"
          }`}
                ...
                <ul className="mt-3 border border-gray-400 mx-4">
                  {item.subItems.map((subItem) => (
                    <Link to={subItem.link}>
                      <li
                        key={subItem.id}
                        className="block text-gray-900 text-sm border border-b-gray-400 items-center gap-x-4 cursor-pointer p-2 px-5  hover:bg-[#C9252C] rounded-md"
                      >
                        {subItem.name}
                      </li>
                    </Link>
